/**
 * Ghost Lock v3.0 - "Hold Open" Mode with Built-in LED testing
 * Logic:
 * 1. Scan -> Connect -> Verify User (Security Check).
 * 2. Disconnect immediately (Stealth).
 * 3. KEEP RELAY HIGH & LED ON (Door Unlocked) while user is detected.
...
// Most ESP32 dev boards have a built-in Blue LED on Pin 2
const int LED_PIN = 2; 
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n=== Ghost Lock v3: Hold-Open Mode (WITH LED) ===");
