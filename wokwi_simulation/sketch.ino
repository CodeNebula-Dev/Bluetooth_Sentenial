...
// 1. Your Phone's "Secret Flag" (Service UUID)
...
const bool ACTIVE_STATE = HIGH; // Set to LOW if using a Relay Module that triggers on Low

// Mock RSSI using a Potentiometer for Wokwi Simulation
  ...
  void onResult(const NimBLEAdvertisedDevice *advertisedDevice) override {
    if (advertisedDevice->isAdvertisingService(serviceUUID)) {
      ...
      if (currentState == SECURE_LOCKED) {
        // --- SIMULATION HACK: Read Potentiometer for RSSI ---
        int potValue = analogRead(POT_PIN);
        int simulatedRSSI = map(potValue, 0, 4095, -100, -40); // Map 0-4095 to -100dBm to -40dBm
        
        Serial.print("Target Detected. Simulated RSSI: ");
        Serial.println(simulatedRSSI);

        if (simulatedRSSI > RSSI_UNLOCK_THRESHOLD) {
          ...
          Serial.println("Target too far. Keeping locked.");
        }
      }
    }
  }
